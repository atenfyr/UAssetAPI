...
public class MonitoringStream : Stream
{
    ...
    /// </summary>
    public static long StopOffset = -1; // 1322439
    /// <summary>
    ...
    {
        if (_asset != null && _asset.IsParsingToPullSchemas) return _innerStream.Read(buffer, offset, count);

        long ourStopOffset = StopOffset;
        if (IsUexpOffset)
        {
            if (_asset == null || _asset.Exports == null || _asset.Exports.Count == 0)
            {
                ourStopOffset = -1;
            }
            else
            {
                ourStopOffset += _asset.Exports[0].SerialOffset;
            }
        }
        if (ourStopOffset >= 0 && _innerStream.Position <= ourStopOffset && (_innerStream.Position + count) > ourStopOffset)
        {
            long correctedPosition = _innerStream.Position;
            if (IsUexpOffset) correctedPosition -= _asset.Exports[0].SerialOffset;
            Debug.WriteLine($"Stop byte reached: reading {count} bytes starting at {correctedPosition}");
            Debugger.Break();
        }
